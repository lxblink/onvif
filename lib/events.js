...
function _terminationTime(response) {
	return Date.now() - response.currentTime.getTime() + response.terminationTime.getTime();
}
			...
			if (!err) {
				if (data.notificationMessage) {
					...
					data.notificationMessage.forEach(function(message) {
						...
						if (message.subscriptionReference.address === this.events.subscription.subscriptionReference.address) {
							/**
